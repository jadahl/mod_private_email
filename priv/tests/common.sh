...
request()
{
    ...
    echo . > $DEBUG_OUTPUT
    echo '###' Running curl: curl -g -X POST "http://$SERVER/$BASE_PATH/$SUB_PATH" \
        $EXTRA -v \
...
get()
{
    ...
    echo . > $DEBUG_OUTPUT
    echo '###' Running curl: curl -g "http://$SERVER/$BASE_PATH/$SUB_PATH?$PARAMS" \
        $EXTRA -v \
        -H "Host: $HOST" > $DEBUG_OUTPUT
    curl -g "http://$SERVER/$BASE_PATH/$SUB_PATH?$PARAMS" \
        $EXTRA -v \
